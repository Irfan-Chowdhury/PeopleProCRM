...
use Modules\CRM\App\Models\InvoiceItem;
...
class InvoiceController extends Controller
{
	public function index()
	{
		...
		if ($logged_user->can('view-invoice'))
		{
			...
			return view('crm::projects.invoices.index');
		}
	...
	public function create()
	{
		...
		return view('crm::projects.invoices.create', compact('projects', 'tax_types', 'invoice_number','items'));
	}
	...
	public function show(Invoice $invoice)
	{
		...
		if (auth()->user()->can('view-invoice')||$invoice->client_id == auth()->user()->id)
		{
			...
			if (auth()->user()->role_users_id == 3){
				return view('crm::client.invoices.invoice_show', compact('invoice', 'project', 'company', 'client','location')); //confuse
			}

			return view('crm::projects.invoices.show', compact('invoice', 'project', 'company', 'client','location'));
		}
		return abort('403', __('You are not authorized'));
	}
	...
	public function edit(Invoice $invoice)
	{
		...
		if (auth()->user()->can('edit-invoice')) {
			...
			return view('crm::projects.invoices.edit', compact('invoice', 'tax_types', 'projects','items'));
		}
		return abort('403', __('You are not authorized'));
	}
